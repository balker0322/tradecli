...
class BinanceFutures:
    ...
    def rm_sltp(self, long_tp_func=None, long_sl_func=None, short_tp_func=None, short_sl_func=None):
        """
        function assignment for sl and tp calculations
        :param long_tp_func: funtion for long tp calculation
        :param long_sl_func: funtion for long sl calculation
        :param short_tp_func: funtion for short tp calculation
        :param short_sl_func: funtion for short sl calculation
        """
        self.sltp_values = {
                            'is_rm_sltp_active': True,
                            'long_tp_func': long_tp_func,
                            'long_sl_func': long_sl_func,
                            'short_tp_func': short_tp_func,
                            'short_sl_func': short_sl_func,
                            }        
    ...
    # risk management sltp implementation
    def eval_rm_sltp(self):
        """
        evaluate simple profit target and stop loss
        """
        
        positionAmt = self.get_position()['positionAmt']
        entryPrice = self.get_position()['entryPrice']
        pos_size = float(positionAmt)
        ...
        is_rm_sltp_active = self.get_sltp_values()['is_rm_sltp_active']   

        avg_entry = self.get_position_avg_price()

        # tp execution logic                
        if is_rm_sltp_active and is_tp_full_size == False:
            if pos_size > 0:       
                tp_price_long = self.get_sltp_values()['long_tp_func'](positionAmt, entryPrice)
                if tp_order is not None:
                    time.sleep(2)                                         
        ...
        if is_rm_sltp_active and is_tp_full_size == False:
            if pos_size < 0:                
                tp_price_short = self.get_sltp_values()['short_tp_func'](positionAmt, entryPrice)
                if tp_order is not None:
                    time.sleep(2)                                                        
                    self.cancel(id=tp_order['clientOrderId'])
                    time.sleep(2)
        ...
        # sl execution logic
        if is_rm_sltp_active and is_sl_full_size == False:
            if pos_size > 0:
                sl_price_long = self.get_sltp_values()['long_sl_func'](positionAmt, entryPrice)
                sl_price_long = float(sl_price_long)
                if sl_order is not None:
                    time.sleep(2)                                    
        ...
        if is_rm_sltp_active and is_sl_full_size == False:
            if pos_size < 0:
                sl_price_short = self.get_sltp_values()['short_sl_func'](positionAmt, entryPrice)
                if sl_order is not None: 
                    time.sleep(2)                                         
                    self.cancel(id=sl_order['clientOrderId'])
                    time.sleep(2)
    ...
    def get_futures_symbol_ticker(self, pair):
        """
        get current price
        :return:
        """
